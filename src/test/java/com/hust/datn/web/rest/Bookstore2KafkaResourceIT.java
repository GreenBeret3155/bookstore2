...
import com.hust.datn.config.KafkaProperties;
...
class Bookstore2KafkaResourceIT {
    ...
    void setup() {
        ...
        Bookstore2KafkaResource kafkaResource = new Bookstore2KafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/bookstore-2-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/bookstore-2-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
